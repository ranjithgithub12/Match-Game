...
class FruitStore extends Component {
  state = {
    score: 0,
    gameTimer: 60,
    displayUrl: imagesList[0],
  ...
  checkTheThumbnailImage = id => {
    ...
    this.setState({displayUrl: imagesList[randomNumber]})
  }
  ...
  getFilterdTabId = () => {
    ...
    const filterTabId = imagesList.filter(
      eachList => eachList.category === isActiveTabId,
    )
  ...
  render() {
    const {score, gameTimer, displayUrl, isActiveTabId, isGameOver} = this.state
    const filterTabId = this.getFilterdTabId()
    console.log(isGameOver)
    return (
      <div className="app-container">
        <NavBar score={score} gameTimer={gameTimer} />
        <div className="app1-container">
          {isGameOver ? (
            <div className="image-tab-list-container">
              <img
              ...
              <ul className="unorder-list-of-tabs">
                {tabsList.map(eachList => (
                  <TabItems
                    eachTabItems={eachList}
                    key={eachList.tabId}
                    isActive={isActiveTabId === eachList.tabId}
                    updateTheActiveTabId={this.updateTheActiveTabId}
                  />
                ))}
              </ul>

              <ul className="unorder-thumbnail-images">
                {filterTabId.map(eachList => (
                  <FruitItems
                    eachTabImage={eachList}
                    key={eachList.id}
                    checkTheThumbnailImage={this.checkTheThumbnailImage}
                  />
                ))}
              </ul>
            </div>
          ) : (
            <div className="trophy-background-image">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png"
                alt="trophy"
                className="trophy-image"
              />
              <p className="your-score-content">Your Score</p>
              <p className="your-score-content">{score}</p>
              <button className="reset-button" onClick={this.onClickPlayAgain}>
                <img
                  src="https://assets.ccbp.in/frontend/react-js/match-game-play-again-img.png"
                  alt="reset"
                />
                <p className="play-again-content">PLAY AGAIN</p>
              </button>
            </div>
          )}
        </div>
